...
const makeMessagesRecvSocket = (config) => {
    const { logger, retryRequestDelayMs, getMessage, sentMessagesCache, shouldIgnoreJid, shouldResendMessageOn475AckError } = config;
    ...
    const handleBadAck = async ({ attrs }) => {
        ...
        if (attrs.error === "475" && !(0, WABinary_1.isJidGroup)(attrs.from)) {
            ...
            if (shouldResendMessageOn475AckError) {
                ...
                const msg = (await (sentMessagesCache === null || sentMessagesCache === void 0 ? void 0 : sentMessagesCache.get(key.id))) || (await getMessage(key));
                if (msg) {
                    logger.trace({ attrs }, "resending message with device_fanout set to false due to 475 ack error");
