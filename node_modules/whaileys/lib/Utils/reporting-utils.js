    ...
    while (i < data.length) {
        ...
        if (wireType === WIRE.BYTES) {
            ...
            if (fieldCfg.m || fieldCfg.children) {
                const sub = extractReportingTokenContent(data.subarray(valStart, valEnd), (_a = fieldCfg.children) !== null && _a !== void 0 ? _a : EMPTY_MAP);
                if (sub === null)
                    return null;
                if (sub.length > 0) {
...
const generateMsgSecretKey = async (modificationType, origMsgId, origMsgSender, modificationSender, origMsgSecret) => {
    ...
    return (0, crypto_2.hkdf)(origMsgSecret, 32, { info: useCaseSecret });
};
const getMessageReportingToken = async (msgProtobuf, message, key) => {
    ...
    if (!content || content.length === 0)
        return null;
    const reportingToken = (0, crypto_1.createHmac)("sha256", reportingSecret)
        .update(content)
        .digest()
        .subarray(0, 16);
    return {
